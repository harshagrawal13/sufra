...
function buildText(order) {
  const { simranName, address, when, phone, items, totalItems, matcha } = order;
  const whenLabel = ({
    asap: 'ASAP',
  ...
  const lines = [];
  lines.push("New Harsh's Kitchen order");
  lines.push('');
  ...
  if (matcha) lines.push(`Matcha pickup: ${matcha}`);
  lines.push('');
  lines.push("— harsh's kitchen");
  return lines.join('\n');
}
...
function buildHtml(order) {
  ...
  return `<!doctype html><html><body style="margin:0;padding:24px;background:#f4f0e8;font-family:-apple-system,Inter,Helvetica,Arial,sans-serif;color:#1c1a16;">
    <div style="max-width:560px;margin:0 auto;background:#faf7f0;padding:32px;border:0.5px solid #1c1a1622;">
      <div style="font-family:Georgia,serif;font-style:italic;font-size:24px;margin-bottom:4px;">Harsh's Kitchen<span style="color:#c0392b;">.</span></div>
      ...
      ${matcha ? `<div style="margin-top:16px;padding:12px 14px;background:#f4f0e8;font-size:13px;color:#1c1a16;">🍵 Matcha pickup: <strong>${esc(matcha)}</strong></div>` : ''}
    </div>
  </body></html>`;
}
...
async function sendViaResend({ subject, text, html }) {
  ...
  const from = process.env.ORDER_EMAIL_FROM || "Harsh's Kitchen <onboarding@resend.dev>";
...
export async function POST(req) {
  ...
  const subject = `Harsh's Kitchen · new order from ${order.simranName} (${order.totalItems} ${order.totalItems === 1 ? 'favor' : 'favors'})`;
